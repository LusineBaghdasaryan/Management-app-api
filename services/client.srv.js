...
class ClientSrv {
    ...
    async createClient(body) {
        ...
        try {
            ...
            const phoneIsExist = await this.model.findOne({phone});
            if (phoneIsExist) {
                const err = new Error(errorMessages.phoneAlreadyExists);
                return AsyncFunctionResponse.constructResponseErrModel(err);
            }
            
            const client = await this.model.findOneAndUpdate({_id: ObjectId()}, body, {
                new: true,
                upsert: true,
                runValidators: true,
    ...
    async getClients() {
        try {
            const clientsResp = await this.model.find().populate('providers').lean();
            if (!clientsResp) {
                const err = new Error(errorMessages.clientNotFound);
                return AsyncFunctionResponse.constructResponseErrModel(err);
            }
            return AsyncFunctionResponse.constructResponseModel(clientsResp.reverse());
        } catch (err) {
            return AsyncFunctionResponse.constructResponseErrModel(err);
        }
    }
    ...
    async getClientById(id) {
        try {
            const clientResp = await this.model.findOne({_id: id}).populate('providers').lean();
            if (!clientResp) {
                const err = new Error(errorMessages.clientNotFound);
                return AsyncFunctionResponse.constructResponseErrModel(err);
            }
    ...
    async updateClient(id, body) {
        ...
        try {
            const client = await this.model.findByIdAndUpdate({_id: id}, body, {new: true})
            .populate('providers').lean();
            if (!client) {
